	  ...
	  //and the distance from this point to the line segment ends in geo1 and geo2 
        ...
        function bdccGeoDistanceToPolyMtrs(poly, pointpassed)
        {
            var d = 999999999;
            var i;
            var p = new bdccGeo(pointpassed.lat,pointpassed.lng);
			var latlngaltArray= poly.getLatLngAltArray();
			var coordinates = [];
			for(i=0; i<(latlngaltArray.length-1); i=i+3)
            {
					var p1 = latlngaltArray[i]+","+latlngaltArray[i+1];
                    var l1 = new bdccGeo(latlngaltArray[i],latlngaltArray[i+1]);
                    var p2 = latlngaltArray[i+3]+","+latlngaltArray[i+4];
                    var l2 = new bdccGeo(latlngaltArray[i+3],latlngaltArray[i+4]);
